...
<br/>
<div class="widget-header"><h5><i class="icon-align-justify"></i> Data Pegawai</h5></div>
            ...
            <div class="table-responsive">
                ...
                <table id="pegawai" class="table table-striped table-bordered table-hover">
                    ...
                    foreach ($data as $pegawai) {
                        echo '<tr id="row_' . $pegawai->nip . '">
						<td>' . ($no++) . '</td>
						<td>' . $pegawai->nip . '</td>
						<td>' . $pegawai->nama . '</td>
						<td>' . $pegawai->deskripsi . '</td>
						<td>' . $pegawai->atasan . '</td>
						<td>' . ucwords($pegawai->unit_kerja) . '</td>
						<td>
							<span class="label label-sm ' . $data_status_icon_map[$pegawai->status] . '">' . $data_status_map[$pegawai->status] . '</span>
						</td>';
                        echo '<td width=80>';
                        if ($has_access && strtolower($pegawai->nip) != strtolower($this->session->userdata('nip'))) {
                            echo '<a title="View Detail" href="pegawai/view/' . $pegawai->nip . '" class="btn btn-xs btn-primary"><i class="icon-eye-open"></i></a>
