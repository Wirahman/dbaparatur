...
class mutil extends CI_Model {
    ...
    public function __construct() {
        ...
        public function access_record($controller, $note='') {
        if (!$this->session->userdata('nip'))
	return;
        $data = array();
        $data['tanggal_aktifitas'] = date('Y-m-d H:i:s');
        $data['nip'] = $this->session->userdata('nip');
        $data['controller'] = $controller;
        $data['ip_aktifitas'] = $this->common->get_host();
        $note = str_replace("'", '', $note);
        $data['data'] = substr($note, 0, 3000); // di DB, varchar2(3000)
        $res = $this->db->insert('history_aktifitas', $data);
		if (!$res) {
        ...
        public function get_access_record($currentnip){
            $res = $this->db->query("SELECT * FROM history_aktifitas WHERE nip = '$currentnip' ORDER BY tanggal_aktifitas DESC");
            return $res;
	...
	public function add_tableref($table, $data, $id_edited) {
		...
		$data['insert_user'] = $this->session->userdata('nip');
	...
	public function read_notif($id, $nip) {
		$this->db->update('t_notifikasi_user', array('read_datetime'=>$this->common->get_mysql_date()), array('id'=>$id, 'ref_username'=>$nip));
	}
	
	public function read_all_notif($nip) {
		$this->db->update('t_notifikasi_user', array('read_datetime'=>$this->common->get_mysql_date()), array('ref_username'=>$nip));
		$this->db->query("UPDATE t_notifikasi_user SET read_datetime=?
		WHERE ref_username=? AND read_datetime IS NULL", array($this->common->get_mysql_date(), $nip));
	}
	...
	public function put_notification($notif, $url, $menus) {
		...
		foreach ($menus as $val) {
			$res = $this->db->query("SELECT nip FROM v_user_menu WHERE menu_url='$val' AND nip<>'$nip'");
			foreach ($res->result() as $row) {
				$arr[] = $row->nip;
			}
		}
