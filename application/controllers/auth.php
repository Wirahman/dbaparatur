        ...
        if (isset($_POST['nip']) && isset($_POST['password'])) {
            ...
            //if ($result_ambilnamapegawai['num_rows'] != 0):
                ...
                if ($result_ambilnamapegawai['num_rows'] != 0):
                    ...
                    $res = $this->muser->auth($this->common->filter($this->input->post('nip')), $this->common->filter($this->input->post('password')), $namapegawaisaatini, 1);
                    if ($res == AJAX_SUCCESS):
                    ...
                    if ($resultcurrent_ambilnamapegawai['num_rows'] != 0):
                        ...
                        $res = $this->muser->auth($this->common->filter($this->input->post('nip')), $this->common->filter($this->input->post('password')), $namapegawaisaatini, 0);
                        if ($res == AJAX_SUCCESS):
                            $this->mutil->access_record(__CLASS__ . '.' . __FUNCTION__, $this->session->userdata('nip'));
                            redirect('dashboard');
                            return;
